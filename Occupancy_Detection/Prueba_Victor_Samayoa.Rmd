...
Primero se analizará el comportamiento de los modelos de acuerdo a diversos valores de corte para asignar la clase a partir de las probabilidades, para esto, se usará la curva ROC.
```{python, engine.path = "/usr/bin/python3"}
# Importamos las librerias a usar en python
import feather
import numpy as np
...
y_prueba = np.array(pd.DataFrame([pd.to_numeric(y_prueba), 1-pd.to_numeric(y_prueba)]).transpose())
rf_fpr = dict()
rf_tpr = dict()
rf_roc_auc = dict()
for i in range(2):
...
plt.figure()
lw = 2
plt.plot(rf_fpr[0], rf_tpr[0], color='darkorange',
         lw=lw, label='ROC curve (area = %0.2f)' % rf_roc_auc[0])
